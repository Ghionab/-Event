...
def event_landing(request, slug):
    ...
    For anonymous users: shows guest checkout registration form.
    For authenticated users: shows ticket preview + redirect CTA to participant portal.
    """
    from registration.forms import RegistrationForm
    ...
    # Only process guest checkout POST for anonymous users
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    registration = form.save(commit=False)
                    registration.event = event
                    registration.user = None  # Guest checkout — no account
